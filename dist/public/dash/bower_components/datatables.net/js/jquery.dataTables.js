  ...
  */
  ...
  function _fnBuildAjax(oSettings, data, fn) {
    ...
    if ($.isPlainObject(ajax) && ajax.data) {
      ajaxData = ajax.data;
      var newData = $.isFunction(ajaxData) ? ajaxData(data, oSettings) :
  ...
  var _selector_run = function _selector_run(type, selector, selectFn, settings, opts) {
    var out = [],
      ...
      j,
      jen,
      selectorType = (0, _typeof2["default"])(selector);
  ...
  var _selector_row_indexes = function _selector_row_indexes(settings, opts) {
    ...
    if (_fnDataSource(settings) == 'ssp') {
      ...
      a = search == 'none' ? displayMaster.slice() :
      // no search
      search == 'applied' ? displayFiltered.slice() :
      // applied search

  ...
  function p(a, g) {
    ...
    if (b && b.data("cke-saved-href")) {
      ...
      if (e) {
        ...
        n.append(m);
        (h = a.widgets.initOn(f, e)) ? (d = a.showNotification(c.embeddingInProgress, "info"), h.loadContent(b, {
          noNotifications: !0,
          callback: function () {
            var b = a.editable().findOne('a[data-cke-autoembed\x3d"' + g + '"]');
            if (b) {
  ...
  CKEDITOR.plugins.add("autoembed", {
    ...
    init: function (a) {
      ...
      a.on("paste", function (c) {
        if (c.data.dataTransfer.getTransferType(a) == CKEDITOR.DATA_TRANSFER_INTERNAL) b = 0;else {

...
(function (factory) {
  ...
  function computeDurationGreatestUnit(duration, durationInput) {
    ...
    if (unit === 'week' && (0, _typeof2["default"])(durationInput) === 'object' && durationInput.days) {
      unit = 'day';
    }
    return unit;
  }
  ...
  */
  ...
  var ParsableModelMixin = {
    ...
    */
    applyRawProps: function applyRawProps(rawProps) {
    ...
    */
    applyManualRawProps: function applyManualRawProps(rawProps) {
      return true;
    },
    applyOtherRawProps: function applyOtherRawProps(rawProps) {
  ...
  */
  var ParsableModelMixin_allowRawProps = function ParsableModelMixin_allowRawProps(propDefs) {
  ...
  var Model = Class.extend(EmitterMixin, ListenerMixin, {
    ...
    setProps: function setProps(newProps) {
      ...
      for (name in newProps) {
        ...
        if ((0, _typeof2["default"])(val) === 'object' || val !== this._props[name]) {
  ...
  DragListener.mixin({
    ...
    setScrollVel: function setScrollVel(topVel, leftVel) {
      ...
      if ((this.scrollTopVel || this.scrollLeftVel) && !this.scrollIntervalId) {
        this.scrollIntervalId = setInterval(proxy(this, 'scrollIntervalFunc'),
        // scope to `this`
  ...
  */
  ...
  var ChronoComponent = Model.extend({
    ...
    addChild: function addChild(chronoComponent) {
    ...
    renderEventsPayload: function renderEventsPayload(eventsPayload) {
    ...
    renderDrag: function renderDrag(eventFootprints, seg) {
      var dragEls = null;
      var children = this.children;
      var i;
      var childDragEls;
    ...
    callChildren: function callChildren(methodName) {
  ...
  */
  ...
  var Grid = FC.Grid = ChronoComponent.extend({
    ...
    setRange: function setRange(unzonedRange) {
    ...
    processRangeOptions: function processRangeOptions() {
      ...
      this.opt('eventTimeFormat') || this.opt('timeFormat') ||
      // deprecated
    ...
    eventFootprintToSegs: function eventFootprintToSegs(eventFootprint, constraintRange) {
  ...
  Grid.mixin({
    ...
    buildBusinessHourSegs: function buildBusinessHourSegs(wholeDay) {
      return this.eventFootprintsToSegs(this.buildBusinessHourEventFootprints(wholeDay));
    },
    // Compute business hour *events* for the grid's current date range.
    // Caller must ask if whole-day business hours are needed.
    // FOR RENDERING
    buildBusinessHourEventFootprints: function buildBusinessHourEventFootprints(wholeDay) {
      var calendar = this.view.calendar;
      return this._buildBusinessHourEventFootprints(wholeDay, calendar.opt('businessHours'));
    },
    _buildBusinessHourEventFootprints: function _buildBusinessHourEventFootprints(wholeDay, businessHourDef) {
      var calendar = this.view.calendar;
  ...
  Grid.mixin({
    ...
    getEventTimeText: function getEventTimeText(eventFootprint, formatStr, displayEnd) {
    ...
    _getEventTimeText: function _getEventTimeText(start, end, isAllDay, formatStr, displayEnd) {
      if (formatStr == null) {
    ...
    getSegCustomClasses: function getSegCustomClasses(seg) {
      var eventDef = seg.footprint.eventDef;
      return [].concat(eventDef.className,
  ...
  */
  Grid.mixin({
    ...
    renderHelperEventFootprints: function renderHelperEventFootprints(eventFootprints, sourceSeg) {
      return this.renderHelperEventFootprintEls(eventFootprints, sourceSeg).addClass('fc-helper');
    },
    renderHelperEventFootprintEls: function renderHelperEventFootprintEls(eventFootprints, sourceSeg) {
      // Subclasses must implement.
  ...
  Grid.mixin({
    ...
    segDragStart: function segDragStart(seg, ev) {
      ...
      this.publiclyTrigger('eventDragStart', {
        context: seg.el[0],
        args: [seg.footprint.getEventLegacy(), ev, {},
        // jqui dummy
        this.view]
      });
    },
  ...
  */
  Grid.mixin({
    ...
    // Must return elements used for any mock events.
    renderEventResize: function renderEventResize(eventFootprints, seg) {
      // subclasses must implement
    },
  ...
  */
  ...
  function getDraggedElMeta(el) {
    ...
    if (eventProps) {
      if ((0, _typeof2["default"])(eventProps) === 'object') {
  ...
  Grid.mixin({
    ...
    */
    ...
    initFillInternals: function initFillInternals() {
      this.elsByFill = {};
    },
  ...
  */
  ...
  DayGrid.mixin({
    ...
    fgSegHtml: function fgSegHtml(seg, disableResizing) {
      ...
      '</span>';
      return '<a class="' + classes.join(' ') + '"' + (eventDef.url ? ' href="' + htmlEscape(eventDef.url) + '"' : '') + (skinCss ? ' style="' + skinCss + '"' : '') + '>' + '<div class="fc-content">' + (this.isRTL ? titleHtml + ' ' + timeHtml :
  ...
  var TimeGrid = FC.TimeGrid = Grid.extend(DayTableMixin, {
    dayRanges: null,
    // UnzonedRange[], of start-end of each day
    slotDuration: null,
    ...
    renderHtml: function renderHtml() {
      var theme = this.view.calendar.theme;
      return '' + '<div class="fc-bg">' + '<table class="' + theme.getClass('tableGrid') + '">' + this.renderBgTrHtml(0) +
      // row=0
    ...
    renderNowIndicator: function renderNowIndicator(date) {
      ...
      var segs = this.componentFootprintToSegs(new ComponentFootprint(new UnzonedRange(date, date.valueOf() + 1),
  ...
  var View = FC.View = ChronoComponent.extend({
    ...
    executeEventsRender: function executeEventsRender(eventsPayload) {
    ...
    triggerEventDrop: function triggerEventDrop(eventInstance, dateDelta, undoFunc, el, ev) {
      this.publiclyTrigger('eventDrop', {
        context: el[0],
        args: [eventInstance.toLegacy(), dateDelta, undoFunc, ev, {},
        // {} = jqui dummy
        this]
      });
    },
    ...
    triggerEventResize: function triggerEventResize(eventInstance, durationDelta, undoFunc, el, ev) {
      this.publiclyTrigger('eventResize', {
        context: el[0],
        args: [eventInstance.toLegacy(), durationDelta, undoFunc, ev, {},
        // {} = jqui dummy
        this]
      });
    },
  ...
  View.mixin({
    ...
    // DEPRECATED
    start: null,
    // use activeUnzonedRange
    end: null,
    // use activeUnzonedRange
    intervalStart: null,
    // use currentUnzonedRange
    intervalEnd: null,
    // use currentUnzonedRange
    ...
    buildDateProfile: function buildDateProfile(date, direction, forceToValid) {
      ...
      var isValid;
      if (forceToValid) {
        date = this.calendar.msToUtcMoment(validUnzonedRange.constrainDate(date),
        // returns MS
        isDateAllDay);
      }
      ...
      if (activeUnzonedRange) {
        date = this.calendar.msToUtcMoment(activeUnzonedRange.constrainDate(date),
        // returns MS
        isDateAllDay);
      }
    ...
    buildRenderRange: function buildRenderRange(currentUnzonedRange, currentRangeUnit) {
  ...
  */
  ...
  var Calendar = FC.Calendar = Class.extend(EmitterMixin, {
    ...
    reportViewDatesChanged: function reportViewDatesChanged(view, dateProfile) {
  ...
  */
  Calendar.mixin({
    ...
    option: function option(name, value) {
      ...
      } else if ((0, _typeof2["default"])(name) === 'object') {
  ...
  Calendar.mixin({
    ...
    buildViewSpecOptions: function buildViewSpecOptions(spec) {
      ...
      this.dirDefaults, this.localeDefaults,
    ...
    buildViewSpecButtonText: function buildViewSpecButtonText(spec, requestedViewType) {
      ...
      spec.buttonTextOverride = queryButtonText(this.dynamicOverrides) || queryButtonText(this.overrides) ||
      // constructor-specified buttonText lookup hash takes precedence
      spec.overrides.buttonText; // `buttonText` for view-specific options is a string

      // highest to lowest priority. mirrors buildViewSpecOptions
      spec.buttonTextDefault = queryButtonText(this.localeDefaults) || queryButtonText(this.dirDefaults) || spec.defaults.buttonText ||
      // a single string. from ViewSubclass.defaults
      queryButtonText(Calendar.defaults) || (spec.duration ? this.humanizeDuration(spec.duration) : null) ||
      // like "3 days"
  ...
  Calendar.mixin({
    ...
    initialRender: function initialRender() {
      ...
      if (this.opt('handleWindowResize')) {
        $(window).resize(this.windowResizeProxy = debounce(
    ...
    windowResize: function windowResize(ev) {
      if (!this.ignoreWindowResize && ev.target === window &&
  ...
  */
  ...
  Calendar.prototype.constraintValToFootprints = function (constraintVal, isAllDay) {
    ...
    } else if ((0, _typeof2["default"])(constraintVal) === 'object') {
  ...
  Calendar.mixin({
    ...
    updateEvent: function updateEvent(eventProps) {
      ...
      if (eventDef instanceof SingleEventDef) {
        ...
        eventDefMutation = EventDefMutation.createFromRawProps(eventInstance, eventProps,
        // raw props
  ...
  */
  var ComponentFootprint = FC.ComponentFootprint = Class.extend({
    unzonedRange: null,
    isAllDay: false,
    // component can choose to ignore this
  ...
  var EventManager = Class.extend(EmitterMixin, ListenerMixin, {
    ...
    stickySource: null,
    otherSources: null,
    // does not include sticky source
  ...
  var EventDef = FC.EventDef = Class.extend(ParsableModelMixin, {
    ...
    id: null,
    // normalized supplied ID
    rawId: null,
    // unnormalized supplied ID
    uid: null,
    // internal ID. new ID for every definition
    ...
    textColor: null,
    className: null,
    // an array. TODO: rename to className*s* (API breakage)
    miscProps: null,
    ...
    buildInstances: function buildInstances(unzonedRange) {
      ...
      while (unzonedDate.isBefore(unzonedEnd)) {
        ...
        if (!this.dowHash || this.dowHash[unzonedDate.day()]) {
          ...
          instances.push(new EventInstance(this,
          // definition
  ...
  */
  var EventInstanceGroup = Class.extend({
    ...
    getAllEventRanges: function getAllEventRanges() {
  ...
  */
  ...
  EventDefMutation.createFromRawProps = function (eventInstance, newRawProps, largeUnit) {
    ...
    for (propName in newRawProps) {
      ...
      (0, _typeof2["default"])(newRawProps[propName]) !== 'object' && propName !== 'start' && propName !== 'end' && propName !== 'allDay' && propName !== 'source' && propName !== '_id') {
  ...
  */
  EventSource.parse = function (rawInput, calendar) {
    ...
    if ((0, _typeof2["default"])(rawInput) === 'object') {
  ...
  var BootstrapTheme = Theme.extend({
    classes: {
      ...
      // avoid `table` class b/c don't want margins. only border color
      tableList: 'table table-striped',
      // `table` class creates bottom margin but who cares
      ...
      // the plain `info` class requires `.table`, too much to ask
  ...
  var agendaTimeGridMethods = {
    ...
    renderHeadIntroHtml: function renderHeadIntroHtml() {
      ...
      if (this.opt('weekNumbers')) {
        ...
        return '' + '<th class="fc-axis fc-week-number ' + view.calendar.theme.getClass('widgetHeader') + '" ' + view.axisStyleAttr() + '>' + view.buildGotoAnchorHtml(
        // aside from link, important for matchCellWidths
        {
          date: weekStart,
          type: 'week',
  ...
  */
  var ListViewGrid = Grid.extend({
    dayDates: null,
    // localized ambig-time moment array
    dayRanges: null,
    // UnzonedRange[], of start-end of each day
    ...
    renderEmptyMessage: function renderEmptyMessage() {
      this.el.html('<div class="fc-list-empty-wrap2">' +
      // TODO: try less wraps
    ...
    dayHeaderHtml: function dayHeaderHtml(dayDate) {
      ...
      return '<tr class="fc-list-heading" data-date="' + dayDate.format('YYYY-MM-DD') + '">' + '<td class="' + view.calendar.theme.getClass('widgetHeader') + '" colspan="3">' + (mainFormat ? view.buildGotoAnchorHtml(dayDate, {
